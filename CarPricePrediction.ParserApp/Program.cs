    ...
    using (var reader = new StreamReader("Quikr_car.csv"))
    {
        ...
        using (var csvReader = new CsvReader(reader, csvConfig))
        {
            ...
            foreach (var x in records)
            {
                ...
                car.Owner = !string.IsNullOrEmpty(x.Owner) ? x.Owner : null;
            ...
            {
                var existingCar = ctx.Cars.FirstOrDefault(c => c.Name == car.Name && c.Label == car.Label && c.Location == car.Location && c.Fuel_type == car.Fuel_type && c.Company == car.Company);
                if (existingCar == null)
                {
                    ctx.Cars.Add(car);
                }
